...
- (void)boot
{
	DebugLog(@"[INFO] %@/%@ (%s.265db0d)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)systemUserAgent
{
    ...
    NSString *kChooseAMonsterUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    
    return [[NSString stringWithFormat:@"%@/%s (%@)",kChooseAMonsterUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a ChooseAMonster app using the node.js CLI
