...
NSString * ChooseAMonster$ModuleRequireFormat = @"(function(exports){"
...
@implementation ChooseAMonsterObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_chooseamonster);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ChooseAMonster global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "ChooseAMonster" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_chooseamonster = [[ChooseAMonsterObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_chooseamonster];

	NSString *_chooseamonsterNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _chooseamonsterNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_chooseamonster valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ChooseAMonster$ModuleRequireFormat, dirname, filename, code];
...
- (TiModule *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.chooseamonster.kroll"
