...
//Rare exceptions to indicate a bug in the _chooseamonster code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_chooseamonster = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_chooseamonster]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.chooseamonster.module"
